...
const POLYGON_INCO_CONTRACT_ADDRESS = "0x4e3A5b8Cbe55C135DDDeda1ede2723baaAa596ab";
const POLYGON_CONTRACT_ADDRESS = "0xB06c49aD34873adE1A26036B82BCb1989a0B8f08";
...
async function callHandleOnPolygon(origin, sender, recipientAddress, message) {
  try {
    console.log(sender)
    const contractToCall = new ethers.Contract(POLYGON_CONTRACT_ADDRESS, POLYGON_ABI, polygonWallet);
...
async function listenToDispatchWithdrawFundsOnPolygon() {
  ...
  polygonContract.on("DispatchWithdrawFunds", async (user) => {
    console.log(`Dispatch event detected on Polygon:
          user: ${user}`);
    try {
...
async function settleDispatchWithdrawOfFundsOnPolygon(user) {
  try {
    ...
    console.error(`handle function called on Inco with tx: ${tx.hash}`);
  } catch (error) {
    console.error("Error calling handle function on Inco:", error);
  }
}
